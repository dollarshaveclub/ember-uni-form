...
test('it should call attrs.action on submit when there is no validation', function (assert) {
  assert.expect(1);
  this.subject({ action: () => assert.ok(true), form: { model: {} } });
  this.subject().submit().then(assert.async());
});

test('it should call attrs.action on submit when validation succeeds', function (assert) {
  assert.expect(1);
  this.subject({ action: () => assert.ok(true), form: { model: { validate: () => new Ember.RSVP.Promise(resolve => resolve()) } } });
  this.subject().submit().then(assert.async());
});

test('it should set submitAborted on submit when model.validate fails', function (assert) {
  this.subject({ action: () => assert.ok(true), form: { model: { validate: () => new Ember.RSVP.Promise((resolve, reject) => reject()) } } });
  var done = assert.async();
  this.subject().submit().then(() => {
    assert.equal(this.subject().get('submitAborted'), true);
    done();
  });
});
...
test('it should call attrs.action on submit when model.validate fails and submitWithErrors is set', function (assert) {
  assert.expect(1);
  this.subject({ action: () => assert.ok(true), form: { model: { validate: () => new Ember.RSVP.Promise((resolve, reject) => reject()) } }, submitWithErrors: true });
  this.subject().submit().then(assert.async());
});
